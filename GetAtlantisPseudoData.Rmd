...
unique(truth$bio_catch$time) # need to adjust time to deal with 90 output periodicity
...
for(i in 1:5){
  print(compare_catchB +
          facet_wrap_paginate(~species, ncol=3, nrow = 2, scales="free", page = i))
}

# For when fisheries output at diff interval from summary output
...
```
Open points are from the Catch.txt file, line from the aggregated catch biomass calculation with scaling from BF Dec 2015
...
# 1. Pull catches for entire system and convert to biomass, summarize over depths
...
for(i in paste(indBDNLookup[indBDNLookup$BNindexGroup == "inverts", "Code"],
               "Catch", sep = "_")){
  search <- c(search, grep(i, catchVars$name, fixed = TRUE, value = TRUE))
  }
...
# 2. pull out just the Georges Bank catches, find ratio
...
# Read in the functional groups csv since that is used by many functions
  ...
  bps <- load_bps(dir = dir, fgs = file_fgs, file_init = file_init)
                   ...
                   file_nc = paste0(scenario, 'CATCH.nc'),
                   bps = bps,
                   fgs = fgs,
...
commSF <- catchGB %>% 
              filter(species %in% indBDNLookup[indBDNLookup$BNindexGroup == "inverts", "Name"]) %>%
              group_by(time) %>% 
              dplyr::summarize(commSF = sum(catchGB))
ggplot() + geom_line(data = commSF, aes(x=time,y=commSF))
```
...
plot(recEffort$Time/365, recEffort$recfish)
```
